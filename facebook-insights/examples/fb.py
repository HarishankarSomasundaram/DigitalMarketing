        ...
        for response_json in response_json_list:
            ...
            for i in range(len(data)):
                ...
                if next_link is not None:
                    r = requests.get(next_link.replace(
                        "limit=25", "limit=100"))
                ...
                if next_link is not None:
                    ...
                    while True:
                        ...
                        try:
                            ...
                            print(
                                "Comments for the post {} completed".format(post_id))
                            break
        return list_all
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            data = fb.get_feed_data(target_page_input, str(
                offset), field_input, path, date_since_input)
            check = data['data']
            if (len(check) >= 100):
    ...
    fb.create_table(feed_table_list, csv_feed_path_input,
                    target_page_input, "feed")
    fb.create_table(likes_table_list, csv_likes_path_input,
                    target_page_input, "likes")
    fb.create_table(comments_table_list, csv_comments_path_input,
                    target_page_input, "comments")
